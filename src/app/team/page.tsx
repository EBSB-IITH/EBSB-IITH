		...
		<div className="flex flex-col">
			<div className="flex flex-col mb-44 *:mx-auto text-background bg-foreground rounded-b-[4rem] lg:rounded-b-[8rem] pt-[2rem] relative" >
				...
				<div className="absolute self-center flex flex-row bottom-[-2rem] bg-white text-foreground text-xl px-6 py-6 rounded-[3rem] drop-shadow-lg *:mx-8">
					...
					<div data-isEvents={isEvents && !isFullTeam} onClick={() => { setIsEvents(false); setIsPR(false); setIsFinance(false); setIsDesign(false); setIsWeb(false); setIsMultimedia(false); setIsEvents(true) }} className="data-[isEvents=true]:font-bold">Events</div>
					<div data-isFinance={isFinance && !isFullTeam} onClick={() => { setIsFinance(false); setIsPR(false); setIsFinance(true); setIsDesign(false); setIsWeb(false); setIsMultimedia(false); setIsEvents(false) }} className="data-[isFinance=true]:font-bold">Finance & Equipment</div>
					<div data-isDesign={isDesign && !isFullTeam} onClick={() => { setIsDesign(false); setIsPR(false); setIsFinance(false); setIsDesign(true); setIsWeb(false); setIsMultimedia(false); setIsEvents(false) }} className="data-[isDesign=true]:font-bold">Design</div>
					<div data-isMultimedia={isMultimedia && !isFullTeam} onClick={() => { setIsMultimedia(false); setIsPR(false); setIsFinance(false); setIsDesign(false); setIsWeb(false); setIsMultimedia(true); setIsEvents(false) }} className="data-[isMultimedia=true]:font-bold">Multimedia</div>
					<div data-isWeb={isWeb && !isFullTeam} onClick={() => { setIsWeb(false); setIsPR(false); setIsFinance(false); setIsDesign(false); setIsWeb(true); setIsMultimedia(false); setIsEvents(false) }} className="data-[isWeb=true]:font-bold">Web</div>
				</div>
			</div>
